import httpx
from ai_framework.Embedder import Embedder, EmbedderAWS
...
from ai_framework.GenAINodeChainRAG import EnumRagStrategy, GenAINodeChainRAG
...
def test_run_graph():
    if mode in ("test", "prod"):
        ...
        Eres un asistente virtual experto en el proceso afiliacion de un colaborador de un banco.\n\nPara cumplir con esta tarea debes de seguir los siguientes pasos: Primero, debes de saludar y solicitar la identificación del usuario. A continuación, con esta identificación debes de validar si el colaborador pertenece al Banco o a una Asociacion. En caso de que el colaborador pertenezca a una Asociación debes de comunicarle que no puedes ayudarle en este proceso de afiliacion porque no pertenece al banco y despedirte. De lo contrario, debes de proceder a validar si el colaborador cumple con los Requisitos de colaborador. En caso de que no los cumpla debes de indicarle que requisitos no cumple, indicarle que por ello no podras continuar y despedirte. De lo contrario, debes de indicarle que puede acceder a un simulador indicandole Infórmate, simula tus aportaciones del plan de ahorro: https://4u.pichincha.com/estatico/plan-futuro-seguro y pidele que despues de la simulacion indique si desea afíliarse de manera simple y guiada. 
        En caso de que el usuario se niegue a afiliarse haz esto>(Pregunta al usuario si le gustaria afiliarse mas adelante. Espera su respuesta. En caso de una respuesta negativa, despidete amenamente. De lo contrario, explicale que un correo, en donde se le recuerde que la afiliacion puede llevarse a acabo del 1 al 10 de cada mes, le será enviado el primer lunes del mes actual o, en su defecto, del siguiente mes.), de lo contrario haz esto>(Indicale los pasos a seguir. 1. Aceptar terminos y condicciones. 2. Completar la documentacion. 3. Entrega fisica de la documentacion. y tambien indicale que la documentacion que debe completar y entregar es la siguiente: 3 copias de contrato impresos a doble cara, Formulario Conozca a su cliente impreso a doble cara, Copia de cédula de ambos lados, Papeleta de votación de ambos lados. Finalmente indicale que puedo hacerlo a través de https://4u.pichincha.com/estatico/plan-futuro-seguro.) y despidete. TOOLS:\n------\n\nAssistant has access to the following tools:\n\n{tools}\n\nTo use a tool, please use the following format:\n\n```\nThought: Do I need to use a tool? Yes\nAction: the action to take, should be one of [{tool_names}]\nAction Input: the input to the action\nObservation: the result of the action\n```\n\nWhen you have a response to say to the Human, or if you do not need to use a tool, you MUST use the format:\n\n```\nThought: Do I need to use a tool? No\nFinal Answer: [your response here]\n```\n\nBegin!\n\nPrevious conversation history:\n{chat_history}\n\nNew human_input: {user_input}\n{agent_scratchpad}
        """
        ...
        node_guardrail_topic = GenAINodeChain(
            ...
            """,
                partials={
                    "system_prompt": """Eres un asistente experto enfocado en temas netamente de recursos humanos de una organizacion como: pólizas de futuro, planificación, nómina, seguro, vacaciones, preguntas frequentes sobre la interacción del empleado y su ciclo de vida en la empresa, salidas de empleado, ausentismos, salida de banco, futuros líderes, etc. Recibirás una entrada de un usuario en '<entrada-usuario>'. Tu tarea es clasificar si esa consulta no es dañina para una organización. Tu respuesta DEBE ser SOLO una de estas opciones:
                1. congruente
                2. incongruente
                Congruente significa que la entrada del usuario no es dañina para la organización. Incongruente significa que la entrada del usuario es dañina para la organización.  
        ...
        graph_test = graph_assistant.run(
            {"user_input": f"hola, háblame sobre el plan futuro seguro"},
